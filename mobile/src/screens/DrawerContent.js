...
import { getInactiveWorkers, updateWorkerStatus, addAdvance } from '../api';
...
export default function DrawerContent({ navigation }) {
  ...
  const onLeave = inactiveWorkers.filter(w => w.status === 'long_leave');
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          {(user?.role === 'manager' || user?.role === 'admin') && navItem('Add New Worker', '➕', 'CreateWorker')}
        </View>
        ...
        {onLeave.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>ON LONG LEAVE</Text>
            {loading ? <ActivityIndicator color={COLORS.primary} /> : onLeave.map(w => (
              <TouchableOpacity key={w._id} style={styles.inactiveItem} onLongPress={() => reactivate(w)}>
                <View style={[styles.inactiveAvatar, { backgroundColor: '#FFF3E0' }]}>
                  <Text style={[styles.inactiveAvatarText, { color: COLORS.amber }]}>{w.name[0]}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.inactiveName}>{w.name}</Text>
                  <Text style={styles.inactiveRole}>{w.longLeaveReason || 'On leave'}</Text>
                </View>
                <View style={[styles.statusPill, { backgroundColor: '#FFF3E0' }]}>
                  <Text style={[styles.statusPillText, { color: COLORS.amber }]}>Leave</Text>
                </View>
              </TouchableOpacity>
            ))}
            {onLeave.length > 0 && <Text style={styles.holdHint}>Hold to reactivate</Text>}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  profileHeader: { backgroundColor: COLORS.primary, paddingTop: 60, paddingBottom: 20, paddingHorizontal: 20, alignItems: 'center' },
