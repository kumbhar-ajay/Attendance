...
import { getTodayAttendance, markAttendance, addAdvance, getActionLog, undoAction, updateWorkerStatus } from '../api';
...
export default function ManagerHome({ navigation }) {
  ...
  const [showLongLeaveModal, setShowLongLeaveModal] = useState(false);
  ...
  useEffect(() => {
    const unsub = NetInfo.addEventListener(state => {
      ...
      else { setNoInternet(false); fetchData(); }
    });
    fetchData();
    return unsub;
  }, []);
  ...
  const fetchData = async () => {
    try {
      const [wRes, logRes] = await Promise.all([getTodayAttendance(), getActionLog()]);
  ...
  const handleMarkAttendance = async (worker, value) => {
    ...
    try {
      const { data } = await markAttendance(worker._id, todayStr, value);
  ...
  const handleAddAdvance = async (worker, amount) => {
    ...
    try {
      await addAdvance(worker._id, Number(amount), todayStr);
      setWorkerField(worker._id, { todayAdvance: (worker.todayAdvance || 0) + Number(amount) });
      const logRes = await getActionLog();
      setUndoLogs(logRes.data.data || []);
  ...
  const handleLongLeave = async () => {
    if (!leaveReason.trim()) { Toast.show({ type: 'error', text1: 'Enter leave reason' }); return; }
    try {
      await updateWorkerStatus(selectedWorker._id, 'long_leave', leaveReason);
      Toast.show({ type: 'success', text1: `${selectedWorker.name} on long leave` });
      setShowLongLeaveModal(false); setShowMenu(false); setLeaveReason(''); fetchData();
    } catch (e) { Toast.show({ type: 'error', text1: 'Failed' }); }
  };
    ...
    return (
      <View style={[styles.workerCard, isMarked && styles.workerCardFaded]}>
        ...
        <View style={styles.advRow}>
          ...
          {QUICK_ADV.map(amt => (
            <TouchableOpacity key={amt} style={styles.advBtn} onPress={() => handleAddAdvance(w, amt)}>
              <Text style={styles.advBtnText}>₹{amt}</Text>
            </TouchableOpacity>
          ))}
          <TouchableOpacity style={styles.advBtn} onPress={() => { setShowAdvInput(w._id); setAdvAmount(''); }}>
  ...
  return (
    <View style={styles.container}>
        ...
        <TouchableOpacity style={styles.modalOverlay} onPress={() => setShowMenu(false)}>
          <View style={styles.bottomSheet}>
            ...
            {[
              ...
              { label: 'Disable Worker', icon: '🚫', color: COLORS.red, action: handleDisable },
              { label: 'Long Leave', icon: '🏠', color: COLORS.amber, action: () => { setShowMenu(false); setShowLongLeaveModal(true); } },
            ].map((item, i) => (
      ...
      {/* Long Leave Reason Modal */}
      <Modal visible={showLongLeaveModal} transparent animationType="fade" onRequestClose={() => setShowLongLeaveModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.alertBox}>
            <Text style={styles.alertTitle}>Long Leave — {selectedWorker?.name}</Text>
            <TextInput style={styles.input} placeholder="Reason (e.g. Village, Family)" value={leaveReason} onChangeText={setLeaveReason} multiline />
            <View style={styles.alertBtns}>
              <TouchableOpacity style={styles.alertBtnCancel} onPress={() => setShowLongLeaveModal(false)}>
                <Text style={styles.alertBtnCancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.alertBtnOk} onPress={handleLongLeave}>
                <Text style={styles.alertBtnOkText}>Confirm</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  undoBar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#FFF3CD', paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#FFE082' },
  ...
  searchInput: { backgroundColor: COLORS.bg, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10, fontSize: 15, borderWidth: 1, borderColor: COLORS.border },
  filterScroll: { backgroundColor: '#fff', paddingVertical: 6, paddingHorizontal: 8, maxHeight: 48 },
