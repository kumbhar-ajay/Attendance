...
import { getManagers, createManager, getReport, adminChangePassword, adminChangeRate } from '../api';
...
export default function AdminHome({ navigation }) {
  ...
  const fetchAll = async () => {
    try {
      const [mRes, rRes] = await Promise.all([getManagers(), getReport(month)]);
      setManagers(mRes.data.data);
      setReport(rRes.data.data);
    } catch (e) { Toast.show({ type: 'error', text1: 'Failed to load' }); }
  ...
  const handleCreateMgr = async () => {
    ...
    setCreating(true);
    try {
      await createManager({ name: mgrName.trim(), mobile: mgrMobile.trim() });
      Toast.show({ type: 'success', text1: `Manager ${mgrName} created!`, text2: `Default password: ${mgrMobile}` });
      setShowCreateMgr(false); setMgrName(''); setMgrMobile(''); fetchAll();
    } catch (e) { Toast.show({ type: 'error', text1: e.response?.data?.message || 'Failed' }); }
        ...
        {managers.map(m => (
          <TouchableOpacity key={m._id} style={styles.mgrCard}
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.mgrSub}>{m.workerCount || 0} workers · {m.mobile}</Text>
            </View>
